...
def set_subscriber_status(user_id: int, status: bool, table_name: str = "users2"):
    try:
        response = supabase.table(table_name).update({'is_subscriber': status}).eq("user_id", user_id).execute()
        return bool(response.data)
    except Exception as e:
        logging.error(f"Error actualizando is_subscriber: {e}")
    return False

def add_priority_days(user_id: int, days: int, table_name: str = "users2") -> bool:
    user = get_user(user_id, table_name)
    if not user:
        return False
    now = datetime.now(timezone.utc)
    expiry_str = user.get('priority_expiry')
    if expiry_str:
        try:
    ...
    try:
        response = supabase.table(table_name).update({'priority_expiry': new_expiry.isoformat()}).eq("user_id", user_id).execute()
        if response.data:
            logging.info(f"Añadidos {days} días de prioridad a {user_id}. Vence: {new_expiry.isoformat()}")
            return True
    except Exception as e:
        logging.error(f"Error add_priority_days: {e}")
    return False
